% This Matlab script is the function "optimize_power_coh_lin" used to generate
...
function [rate1,rate2,check,check2, powers_record, etas_record]=...
    optimize_power_coh_lin(K,L,B,C,D,Pl,Ik,rhoD,tau_u,tau_p,...
    tau_d,effic,rhoP)
...
while (diff>0.001)&&(t>0.001)
    ...
    for k = 1:K
        ...
        scale4*(tau_p*rhoP2+tau_u*eta(k,1))<=scale4*scale2*tau_d*effic*vec(Ik(k,:,:,:))'*vec(PP);
    end
    for l = 1:L
