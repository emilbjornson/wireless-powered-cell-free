% This Matlab script is the function "optimize_power_noncoh_nonlin" used to generate
...
function [rate1,rate2,check,check2,powers_record, etas_record]=...
    optimize_power_noncoh_nonlin(K,L,B,C,D,Pl,Ik,rhoD,tau_u,tau_p,...
...
while  (diff>0.001)&&(t>0.001)
    ...
    for k = 1:K
        ...
        norm( [eee(k,1); Brec*vec(Ik(k,:,:))'*vec(powerr)+Crec; sqrt(2) ])<= eee(k,1)+Brec*vec(Ik(k,:,:))'*vec(powerr)+Crec;
    end
