...
%Rectifier models for two scenarios
modelSet = [1 2];

%Prepare to save simulation results
%for saving SE values
LMMSE_mmf_coh_nonlin = zeros(K,nbrOfSetups,2);
LMMSE_mmf_noncoh_nonlin = zeros(K,nbrOfSetups,2);

LS_mmf_coh_nonlin = zeros(K,nbrOfSetups,2);
LS_mmf_noncoh_nonlin = zeros(K,nbrOfSetups,2);
...
for scen = 1:2
    ...
    if model==1
        ...
        B_nonlin = 1e+6*c;
        C_nonlin = 1000*c^2;
        %The following scalings do not effect the harvested energy, introduced only for
        %preventing numerical issues
        A_nonlin = A_nonlin/500;
        B_nonlin = B_nonlin/500;
        C_nonlin = C_nonlin/500;
    end
