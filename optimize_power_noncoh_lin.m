% This Matlab script is the function "optimize_power_noncoh_lin" used to generate
...
function [rate1,rate2,check,check2,powers_record, etas_record]=...
    optimize_power_noncoh_lin(K,L,B,C,D,Pl,Ik,rhoD,tau_u,tau_p,...
    tau_d,effic,rhoP)
...
scale3 = 1/norm(vec(Pl))*L;
Pl = Pl*scale3;
Ik = Ik*scale3;

scale4 = 1/norm(vec(Ik))*K;
...
while (diff>0.001)&&(t>0.001)
    ...
    for k = 1:K
        ...
        scale4*(tau_p*rhoP2+tau_u*eta(k,1))<=scale2*scale4*effic*tau_d*vec(Ik(k,:,:))'*vec(powerr);
    end
    for l=1:L
        Pl(l,:)*(powerr(l,:)).'/rhoD<=1;
    end
    ...
    if cvx_status(1)=='S'
        maxx = 0;
        for l = 1:L
            maxx = max(maxx,Pl(l,:)*(powerr(l,:)).');
            
        end
        
        if maxx<rhoD
            powerr = powerr*rhoD/maxx;
        end
        ...
        for k = 1:K
            if (tau_p*rhoP2+tau_u*eta(k,1))>scale2*tau_d*effic*vec(Ik(k,:,:))'*vec(powerr)
                eta(k,1) = (scale2*tau_d*effic*vec(Ik(k,:,:))'*vec(powerr)-tau_p*rhoP2)/tau_u;
            end
        end
        
        maxx = 0;
        for k = 1:K
            maxx = max( eta(k,1)/((scale2*tau_d*effic*vec(Ik(k,:,:))'*vec(powerr)-tau_p*rhoP2)/tau_u),maxx);
        end
        if maxx<1
            eta = eta/maxx;
        end
...
if check>0
    ...
    for k = 1:K
        if (tau_p*rhoP2+tau_u*etas(k,1))>scale2*tau_d*effic*vec(Ik(k,:,:))'*vec(powers)
            etas(k,1)=(scale2*tau_d*effic*vec(Ik(k,:,:))'*vec(powers)-tau_p*rhoP2)/tau_u;
        end
    end
    if min(etas)<0
        check = 0;
