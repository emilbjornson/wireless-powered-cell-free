% This Matlab script generates Figures 6 and 7 in the paper:
%
...
%Number of antennas per AP for four different scenarios
NSet = [32 18 8 16];
...
LMMSE_mmf_coh_nonlin = zeros(K,nbrOfSetups,4);
LMMSE_mmf_noncoh_nonlin = zeros(K,nbrOfSetups,4);

LS_mmf_coh_nonlin = zeros(K,nbrOfSetups,4);
LS_mmf_noncoh_nonlin = zeros(K,nbrOfSetups,4);
...
for scen = 1:4
    ...
    while n<nbrOfSetups
        ...
        %problems
        check1=[ check_lmmse_noncoh_nonlin1, ...
        ...
        if (min(check1)>0.5)
            ...
            LMMSE_mmf_noncoh_nonlin(:,n,scen) = prelogFactor*rate_lmmse_noncoh_nonlin1;
            
            
            LS_mmf_coh_nonlin(:,n,scen) = prelogFactor*rate_ls_coh_nonlin1;
            LS_mmf_noncoh_nonlin(:,n,scen) = prelogFactor*rate_ls_noncoh_nonlin1;
...
% Plot Fig. 6
figure
ecdf(vec(LMMSE_mmf_noncoh_nonlin(:,:,1)))
hold on
ecdf(vec(LMMSE_mmf_coh_nonlin(:,:,1)))
ecdf(vec(LMMSE_mmf_noncoh_nonlin(:,:,2)))
ecdf(vec(LMMSE_mmf_coh_nonlin(:,:,2)))
ecdf(vec(LMMSE_mmf_noncoh_nonlin(:,:,3)))
ecdf(vec(LMMSE_mmf_coh_nonlin(:,:,3)))
ecdf(vec(LMMSE_mmf_noncoh_nonlin(:,:,4)))
ecdf(vec(LMMSE_mmf_coh_nonlin(:,:,4)))

% Plot Fig. 7
figure
ecdf(vec(LS_mmf_noncoh_nonlin(:,:,1)))
hold on
ecdf(vec(LS_mmf_coh_nonlin(:,:,1)))
ecdf(vec(LS_mmf_noncoh_nonlin(:,:,2)))
ecdf(vec(LS_mmf_coh_nonlin(:,:,2)))
ecdf(vec(LS_mmf_noncoh_nonlin(:,:,3)))
ecdf(vec(LS_mmf_coh_nonlin(:,:,3)))
ecdf(vec(LS_mmf_noncoh_nonlin(:,:,4)))
ecdf(vec(LS_mmf_coh_nonlin(:,:,4)))
